...
有一个用户态程序在系统里（由 `tools/play_tetris.c` 编译安装）：

```sh
play_tetris
```

至于它会“呈现出什么”，我建议你亲自跑一遍再回来读源码 —— 体验会更好。
...
- `tools/` 目前输出文件名是 `play_tetris.a` 但它其实是可执行文件；你可以顺手把命名和安装逻辑整理得更合理。
